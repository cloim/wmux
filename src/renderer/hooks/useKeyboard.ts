  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (ctrl && !shift && !alt && key === 't') {
        ...
        const ws = state.workspaces.find((w) => w.id === state.activeWorkspaceId);
        if (ws) {
          window.electronAPI.pty.create(withDefaultPtyOptions({ workspaceId: ws.id }, state.defaultShell, state.defaultCwd)).then((result: { id: string; cwd?: string }) => {
            store.getState().addSurface(ws.activePaneId, result.id, 'Terminal', result.cwd || '');
          });
        }
        return;
      }
